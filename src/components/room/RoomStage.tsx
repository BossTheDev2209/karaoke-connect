...
export const RoomStage: React.FC<RoomStageProps> = ({
  ...
  const phaseBadge = showReadyCheck
    ...
    : showCountdown
      ? { label: 'Countdown', className: 'bg-neon-orange/20 text-neon-orange border-neon-orange/30' }
      : showNoSong
        ? { label: 'Idle', className: 'bg-muted text-muted-foreground border-border' }
        : isPaused
          ? { label: 'Paused', className: 'bg-neon-yellow/20 text-neon-yellow border-neon-yellow/30' }
          : isPlaying
  ...
  return (
    <div className="lg:col-span-6 flex flex-col gap-3 min-h-0 order-1 lg:order-2">
      <div className="card-karaoke relative flex-1 min-h-0 overflow-hidden rounded-lg">
        ...
        {showSingReactOverlay && <SingReactOverlay {...singReactOverlayProps} />}

        {showCountdown && (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <div className="rounded-2xl bg-card/70 backdrop-blur border border-border shadow-lg px-6 py-4">
              <div className="text-6xl font-black text-primary tabular-nums text-center">{remainingSeconds}</div>
              <div className="mt-1 text-[10px] uppercase tracking-[0.3em] text-muted-foreground text-center">seconds</div>
            </div>
          </div>
        )}
                ...
                {readyCheckUsers.map((u) => (
                  <div
                    ...
                    className={cn(
                      ...
                      u.isReady ? 'bg-neon-green/20 text-neon-green border border-neon-green/30' : 'bg-muted text-muted-foreground border border-border'
                    )}
                  >
                    {u.isReady && <span className="text-neon-green">✓</span>}
                    {u.nickname}
                  </div>
                ))}
              </div>
              {isHost && (
                <Button variant="default" size="sm" onClick={onForceStart} className="mt-2">
                  Start Now
                </Button>
              )}
            </div>
        ...
        {showRecommendations && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-background/80 backdrop-blur-md rounded-lg z-20 p-6 animate-in fade-in duration-300">
            <div className="text-center mb-6">
              <h3 className="text-xl font-bold text-foreground mb-2">Up Next</h3>
              <p className="text-muted-foreground text-sm">Based on your last song</p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 w-full max-w-3xl">
              {recommendations.map((rec) => (
                <button
                  key={rec.id}
                  type="button"
                  className="group relative aspect-video bg-background/50 rounded-xl overflow-hidden cursor-pointer hover:ring-2 hover:ring-primary transition-all text-left"
                  onClick={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
                ...
                >
                  ...
                  <div className="absolute inset-0 flex flex-col justify-end p-3 bg-gradient-to-t from-background/95 to-transparent">
                    <p className="text-foreground font-medium text-sm line-clamp-2 leading-tight">{rec.title}</p>
                    <p className="text-muted-foreground text-xs mt-1 truncate">{rec.artist}</p>
                  </div>
            ...
            <Button variant="ghost" className="mt-8 text-muted-foreground hover:text-foreground" onClick={onDismissRecommendations}>
              Cancel
            </Button>
          </div>
        )}
        ...
        {showPlayerError && playerError && (
          <div className="absolute inset-0 flex items-center justify-center bg-card/95 rounded-lg backdrop-blur-sm z-10">
            <div className="text-center p-6 max-w-md">
              ...
              <p className="text-sm text-muted-foreground mb-4">{playerError.message}</p>
              <div className="flex flex-col sm:flex-row gap-2 justify-center">
                <a
                  href={`https://www.youtube.com/watch?v=${playerError.videoId}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center justify-center gap-2 px-4 py-2 bg-destructive hover:bg-destructive/90 text-destructive-foreground rounded-lg transition-colors text-sm font-medium"
                >
              ...
              <p className="text-xs text-muted-foreground mt-3">
                {playerError.isAgeRestricted ? 'Age-restricted videos can only be watched directly on YouTube' : 'This video cannot be played in an embedded player'}
              </p>
            </div>
          </div>
        )}
